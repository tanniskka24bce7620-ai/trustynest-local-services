...
const ServiceProviderCard = ({ provider, onViewProfile, emergencyMode }: Props) => {
  ...
  return (
    <div className={`group rounded-xl border bg-card p-5 shadow-soft transition-all hover:shadow-card ${emergencyMode && provider.emergencyAvailable ? "border-destructive/40 ring-1 ring-destructive/20" : "border-border"}`}>
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex items-center gap-1">
            ...
            <span className="text-xs text-muted-foreground">({provider.reviewCount} {t("providerCard.reviews")})</span>
          </div>
        </div>
      </div>
      ...
      {!provider.available && (
        <div className="mt-2 rounded-md bg-destructive/10 px-3 py-1 text-center text-xs font-medium text-destructive">{t("providerCard.currentlyUnavailable")}</div>
      )}
    </div>
  );
};
